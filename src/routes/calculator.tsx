...
export const Route = createFileRoute("/calculator")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://layagreenenergy.com/calculator" },
    ...
    links: [{ rel: "canonical", href: "https://layagreenenergy.com/calculator" }],
  }),
...
function Calculator() {
  ...
  const {
    recommendedSystemKw: size,
    generationKwhPerMonth: generation,
    monthlySavings: savings,
    annualSavings,
    investment,
    paybackYears: payback,
    lifetimeSavings,
    co2AvoidedTonnes: co2,
    ...
    <>
      ...
      <div className="container-page grid gap-10 py-12 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-7">
          <div className="grid grid-cols-3 gap-2 border-b border-border pb-4">
            {(
              ...
              <button
                ...
                className={`border px-3 py-2 text-sm ${mode === value ? "border-primary bg-primary text-primary-foreground" : "border-border"}`}
              >
                {label}
              </button>
            ))}
          ...
          {mode === "bill" && (
            <Field
              label={`Monthly electricity bill: ${formatCurrency(bill)}`}
              id="bill"
              value={bill}
              min={1000}
              max={200000}
          ...
          {mode === "units" && (
            <Field
              label={`Monthly consumption: ${units.toLocaleString("en-IN")} kWh`}
              id="units"
              value={units}
              min={100}
              max={30000}
          ...
          {mode === "roof" && (
            <Field
              label={`Available roof: ${roof.toLocaleString("en-IN")} ${roofUnit}`}
              id="roof"
              value={roof}
              min={100}
              max={5000}
          ...
          {mode === "roof" && (
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setRoofUnit("sqft")}
                className={`border px-3 py-1.5 text-sm ${roofUnit === "sqft" ? "border-primary text-primary" : "border-border"}`}
              >
                Square feet
              </button>
              <button
                type="button"
                onClick={() => setRoofUnit("sqm")}
                className={`border px-3 py-1.5 text-sm ${roofUnit === "sqm" ? "border-primary text-primary" : "border-border"}`}
              >
                Square metres
              </button>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-medium">
              ...
              <select
                ...
                className="mt-2 w-full border border-input bg-background px-3 py-2.5 font-normal"
              >
                {STATES.map((item) => (
          ...
          <div>
            <label htmlFor="tariff" className="mb-2 block text-sm font-medium">
              Electricity unit cost: {formatCurrency(tariff)} / kWh
            </label>
            <input
              id="tariff"
              type="range"
          ...
          <div className="flex items-center gap-3 border border-border p-3 text-sm">
            <input
              id="subsidy"
              type="checkbox"
            ...
            <label htmlFor="subsidy" className="cursor-pointer">
              Include residential subsidy estimate
            </label>
          </div>
          <div className="border-l-2 border-primary/50 bg-muted p-4 text-sm">
            <p className="font-medium">
              {subsidyEligible ? "Residential subsidy included" : "Subsidy guidance"}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              {subsidyEligible
                ? `This estimate includes up to ${formatCurrency(subsidyAmount)} in indicative central support. Confirm current scheme rules and eligibility with the official portal.`
                : "Central rooftop subsidy estimates apply only to eligible residential consumers. Commercial and industrial projects should explore state schemes, financing and tax treatment instead."}
            </p>
          </div>
        </section>
        ...
        <div className="border border-border bg-card p-7">
          ...
          <dl className="mt-6 divide-y divide-border">
            ...
            <Result
              term="Estimated generation"
              value={`${generation.toLocaleString("en-IN")} units / month`}
            />
          ...
          <p className="mt-5 text-xs leading-relaxed text-muted-foreground">
            Estimates use indicative reference values for tariff, generation and installed cost.
            Actual figures depend on site conditions, load pattern, tariff category and equipment
            selected. This is not a guarantee or a quotation.
          </p>
          ...
          {showLead && (
            <form onSubmit={requestReport} className="mt-6 space-y-3 border-t border-border pt-5">
              <p className="text-sm font-medium">Where should we send your follow-up?</p>
              <input
                required
                value={lead.name}
              ...
              <button
                type="submit"
                className="w-full border border-primary px-4 py-2.5 text-sm font-medium text-primary"
              >
                Request follow-up
              </button>
              {leadMessage && <p className="text-xs text-muted-foreground">{leadMessage}</p>}
            </form>
          )}
        </div>
      </div>
